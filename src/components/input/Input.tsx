...
export function Input({ id, placeholder, value, icon, additionalClasses, onChange }: InputProps) {
  ...
  return (
    <div className={`flex items-center h-[50px] bg-[var(--neutral-800)] shadow-[0_0_10px_rgba(0,0,0,0.075)] rounded-[var(--radius-16)] pl-[var(--spacing-16)] ${additionalClasses}`}>
      {displayIcon}
      <input id={id} className="w-full text-(length:--fs-14) border-0 text-[var(--neutral-0)] outline-none rounded-[var(--radius-16)] placeholder:text-[var(--neutral-200)] pl-[var(--spacing-16)]" placeholder={placeholder} value={value} spellCheck="false" autoComplete="off" onChange={onChange} />
    </div>
  );
}
